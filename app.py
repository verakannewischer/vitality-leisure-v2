    ...
    if wx_df is not None:
        ...
        <div style="text-align:center; margin-bottom:40px;">
            <span style="background:#0f2340; border:1px solid #1e3a5f; border-radius:20px;
                ...
                {_tc_icon(t['temp_category'])} {t['temp_avg']:.0f}°C
                {(' &nbsp;·&nbsp; ' + tags) if tags else ''}
            </span>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with h1:
        hday_str = ('<span style="color:#fbbf24">🎉 Public holiday</span>'
                    if top["is_public_holiday"] else
                    '<span style="color:#38bdf8">🏫 School holiday period</span>'
                    if top["is_school_holiday"] else "")
        st.markdown(f"""
        <div class="card" style="border:2px solid #0ea5e9; padding:28px 30px;">
            ...
            <div style="font-size:1rem; color:#e2e8f0; margin-top:10px;">
                ...
                💧 ~{top['visitors']:,} visitors expected
            </div>
            {('<div style="margin-top:6px;">'+hday_str+'</div>') if hday_str else ''}
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("*Tell me how you feel and I'll build your perfect spa day.*")
    ...
    if cohere_ok:
        ...
        SYSTEM_PROMPT = f"""You are a warm, knowledgeable wellness coach at Vitality Leisure Park, 
a premium thermal spa. Your role: create personalised spa visit plans for guests.

CURRENT 7-DAY VISITOR FORECAST:
{forecast_text}

FACILITY OFFERINGS:
- Thermal pools (indoor & outdoor): relaxation, muscle recovery, circulation
- Finnish sauna & steam room: detox, stress relief, skin cleansing  
- Cold plunge pool: post-workout recovery, energy boost, circulation
- Relaxation lounge: quiet rest, reading, mental reset
- Massage treatments (book in advance): deep tissue, hot stone, lymphatic drainage
- Healthy restaurant: soups, grain bowls, smoothies, warm seasonal dishes

YOUR APPROACH:
1. First, ask the guest how they feel and what they need today. Keep it warm and conversational. Ask max 1-2 questions at a time.
2. Once you understand their needs, give a concrete personalised plan:
   - Best day to visit from the forecast (justify using crowd level, weather, holidays)
   - A treatment sequence with suggested durations (e.g. 20 min sauna → cold plunge → 45 min thermal pool)
   - A restaurant recommendation
3. Always use the ACTUAL forecast numbers. Never invent visitor counts.
4. If they ask "what if I come on Saturday?" or change preferences, adapt the plan.
5. Be concise — your responses should be helpful, not overwhelming.

Do not reveal that you are an AI language model or mention Cohere."""
        ...
        if "wellness_history" not in st.session_state:
            ...
            st.session_state.wellness_history.append({
                ...
                "content": (
                    ...
                    "and our live crowd forecast for the next 7 days.\n\n"
